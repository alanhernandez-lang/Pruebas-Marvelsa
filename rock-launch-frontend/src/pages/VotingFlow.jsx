...
function VotingFlow() {
    ...
    const COOLDOWN_SEC = 0; // 0 segundos (desactivado)

    useEffect(() => {
        const TARGET_DATE = new Date("Jan 1, 2024 00:00:00");
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                                    {p.photo_url ? (
                                        <div className="presenter-photo-container" onClick={() => setZoomedPhoto(p)}>
                                            <img src={p.photo_url.trim().startsWith('data:') ? p.photo_url.trim() : `${BASE_API_URL}${p.photo_url.trim()}`} alt={p.name} style={{ width: '50px', height: '50px', borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--accent)' }} />
                                        </div>
                                    ) : (
            ...
            {zoomedPhoto && (
                <div className="image-zoom-overlay" onClick={() => setZoomedPhoto(null)}>
                    <div className="image-zoom-content">
                        <img src={zoomedPhoto.photo_url.trim().startsWith('data:') ? zoomedPhoto.photo_url.trim() : `${BASE_API_URL}${zoomedPhoto.photo_url.trim()}`} alt={zoomedPhoto.name} />
